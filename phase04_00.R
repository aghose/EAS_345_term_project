...
mortgage_data <- read.csv("data/hmda_2017_nationwide_all-records_labels.csv",
                          nrows = 250000) #limiting rows because file too big
"TODO:"
...
#Dropping rows which have no data about applicant's income
#Dropping rows which have no data bout loan amount requested
intermediate_mortgage_data <- mortgage_data %>%
  subset(applicant_race_1!=6 
...
mortgage_data[,6] <- lapply(6, function(x) as.factor(mortgage_data[[x]]))
mortgage_data[,5] <- lapply(5, function(x) as.factor(mortgage_data[[x]]))
intermediate_mortgage_data[,6] <- lapply(6, function(x) as.factor(intermediate_mortgage_data[[x]]))
